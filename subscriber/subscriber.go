...
func (sub *Subscriber) ProcessSubscribe(ethClient *ethClient.EthClient) {
	for {
		...
		case header:= <-sub.header:
			...
			go func() {db.QueryRow("Insert into blocks (blockHeight, receipient, reward, size, gasUsed, hash) values ( ? ? ? ? ? ? )", block.Number(), block.Coinbase(), block.Coinbase(), block.Size(), block.GasUsed(), block.Hash())}
			return
